...
public class MsgStreamer extends OutputStream
  ...
  MsgStreamer(List<Connection> cons, DistributionMessage msg, boolean directReply, DMStats stats,
    ...
    this.msg = msg;
    this.cons = cons;
    int bufferSize = Math.min(sendBufferSize, InternalConnection.MAX_MSG_SIZE);
    buffer = bufferPool.acquireDirectSenderBuffer(bufferSize);
    buffer.clear();
    buffer.position(InternalConnection.MSG_HEADER_BYTES);
      ...
      if (versionToConnMap == null) {
        ...
        if (numCons > numVersioned) {
          ...
          for (Connection c : cons) {
            version = ((InternalConnection) c).getRemoteVersion();
            if (version == null || version.ordinal() >= KnownVersion.CURRENT_ORDINAL) {
              currentVersionConnections.add(c);
            }
          }
          streamers.add(
  ...
  public void reserveConnections(long startTime, long ackTimeout, long ackSDTimeout) {
    for (final Connection connection : cons) {
      connection.setInUse(true, startTime, ackTimeout, ackSDTimeout, cons);
      if (ackTimeout > 0) {
        ((InternalConnection) connection).scheduleAckTimeouts();
      }
    }
  }
  ...
  public void realFlush(boolean lastFlushForMessage) {
    if (isOverflowMode()) {
      if (overflowBuf == null) {
        overflowBuf = new HeapDataOutputStream(
            buffer.capacity() - InternalConnection.MSG_HEADER_BYTES, KnownVersion.CURRENT);
      }
      return;
    }
    buffer.flip();
    ...
    for (Iterator<Connection> it = cons.iterator(); it.hasNext();) {
      InternalConnection con = (InternalConnection) it.next();
      try {
  ...
  public void close() throws IOException {
    try {
      if (startedSerializingMsg && !doneWritingMsg) {
        ...
        if (flushedBytes > 0) {
          for (final Connection connection : cons) {
            InternalConnection con = (InternalConnection) connection;
  ...
  private void setMessageHeader() {
    ...
    if (doneWritingMsg) {
      if (normalMsg) {
        msgType = InternalConnection.NORMAL_MSG_TYPE;
      } else {
        msgType = InternalConnection.END_CHUNKED_MSG_TYPE;
      }
      if (directReply) {
        msgType |= InternalConnection.DIRECT_ACK_BIT;
      }
    } else {
    ...
    buffer.putInt(InternalConnection.MSG_HEADER_SIZE_OFFSET,
        InternalConnection.calcHdrSize(buffer.limit() - InternalConnection.MSG_HEADER_BYTES));
    buffer.put(InternalConnection.MSG_HEADER_TYPE_OFFSET, (byte) (msgType & 0xff));
    buffer.putShort(InternalConnection.MSG_HEADER_ID_OFFSET, msgId);
    buffer.position(0);
  }
