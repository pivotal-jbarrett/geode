...
public class MsgOutputStream extends OutputStream implements ObjToByteArraySerializer {
  ...
  public MsgOutputStream(int allocSize) {
    ...
    this.buffer.position(InternalConnection.MSG_HEADER_BYTES);
  }
  ...
  private int size() {
    return this.buffer.position() - InternalConnection.MSG_HEADER_BYTES;
  }
  ...
  public void setMessageHeader(int msgType, int processorType, short msgId) {
    buffer.putInt(InternalConnection.MSG_HEADER_SIZE_OFFSET,
        InternalConnection.calcHdrSize(size()));
