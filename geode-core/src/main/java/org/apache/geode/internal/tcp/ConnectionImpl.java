...
public class ConnectionImpl implements Runnable, InternalConnection {
   ...
   *
   * TODO jbarrett - this can be replaced with LinkedBlockingQueue now
   */
  ...
  static {
    ...
    byte[] bytes = new byte[InternalConnection.MSG_HEADER_BYTES + msglen];
    msglen = InternalConnection.calcHdrSize(msglen);
    bytes[InternalConnection.MSG_HEADER_SIZE_OFFSET] = (byte) (msglen / 0x1000000 & 0xff);
    bytes[InternalConnection.MSG_HEADER_SIZE_OFFSET + 1] = (byte) (msglen / 0x10000 & 0xff);
    bytes[InternalConnection.MSG_HEADER_SIZE_OFFSET + 2] = (byte) (msglen / 0x100 & 0xff);
    bytes[InternalConnection.MSG_HEADER_SIZE_OFFSET + 3] = (byte) (msglen & 0xff);
    bytes[InternalConnection.MSG_HEADER_TYPE_OFFSET] = (byte) InternalConnection.NORMAL_MSG_TYPE; // message
                                                                                                  // type
    bytes[InternalConnection.MSG_HEADER_ID_OFFSET] = (byte) (MsgIdGenerator.NO_MSG_ID >> 8 & 0xff);
    bytes[InternalConnection.MSG_HEADER_ID_OFFSET + 1] = (byte) (MsgIdGenerator.NO_MSG_ID & 0xff);
    bytes[InternalConnection.MSG_HEADER_BYTES] = REPLY_CODE_OK;
   ...
   */
  protected ConnectionImpl(ConnectionTable connectionTable, Socket socket)
  ...
  private void sendOKHandshakeReply() throws IOException, ConnectionException {
    ...
    if (isReceiver) {
      ...
      bb.putInt(0, InternalConnection.calcHdrSize(bb.position() - MSG_HEADER_BYTES));
  ...
  static ConnectionImpl createSender(final Membership<InternalDistributedMember> mgr,
    ...
    try {
      ...
      while (!success) { // keep trying
        ...
        try {
          conn = null;
          conn = new ConnectionImpl(t, preserveOrder, remoteAddr, sharedResource);
   ...
   * must accept us We will almost always send messages; small acks are received.
   */
  private ConnectionImpl(ConnectionTable t, boolean preserveOrder,
  ...
  private String p2pReaderName() {
    ...
    if (isReceiver) {
      sb.append(Connection.THREAD_KIND_IDENTIFIER).append("@");
    ...
    if (isReceiver && handshakeRead && !preserveOrder && sharedResource) {
      if (!owner.getConduit().getCancelCriterion().isCancelInProgress()) {
        owner.getDM().getDistribution().suspectMember(getRemoteAddress(),
            InternalConnection.INITIATING_SUSPECT_PROCESSING);
      }
    }
  }
  ...
  private static boolean validMsgType(int msgType) {
    return msgType == InternalConnection.NORMAL_MSG_TYPE
        || msgType == InternalConnection.CHUNKED_MSG_TYPE
        || msgType == InternalConnection.END_CHUNKED_MSG_TYPE;
  }
  ...
  @Override
  public void sendPreserialized(@NotNull ByteBuffer buffer, boolean cacheContentChanges,
      @NotNull DistributionMessage msg) throws IOException, ConnectionException {
    if (!connected) {
      ...
      ackTimeoutTask = new SystemTimer.SystemTimerTask() {
        ...
        public void run2() {
          ...
          synchronized (ConnectionImpl.this) {
            if (socketInUse) {
              switch (connState) {
                case ConnectionImpl.STATE_IDLE:
                case ConnectionImpl.STATE_POST_SENDING:
                case ConnectionImpl.STATE_RECEIVED_ACK:
                  break;
                case ConnectionImpl.STATE_SENDING:
                case ConnectionImpl.STATE_READING_ACK:
  ...
  private boolean doSevereAlertProcessing() {
    ...
    if (!ackTimedOut && 0 < ackWaitTimeout
      ...
      final String state = connectionState == ConnectionImpl.STATE_SENDING
  ...
  @Override
  public void readAck(final @NotNull DirectReplyProcessor processor)
  ...
  private void setThreadName(int dominoNumber) {
    Thread.currentThread().setName(Connection.THREAD_KIND_IDENTIFIER + " for " + remoteAddr + " "
