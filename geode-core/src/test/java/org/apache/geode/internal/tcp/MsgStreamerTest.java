...
public class MsgStreamerTest {
  ...
  InternalConnection connection1 = mock(InternalConnection.class);
  InternalConnection connection2 = mock(InternalConnection.class);
  ...
  public void streamerRespectsMaxMessageSize() {
    ...
    message.setRecipients(singletonList(member1));
    ...
    // a buffer
    when(connection1.getSendBufferSize()).thenReturn(ConnectionImpl.MAX_MSG_SIZE + 1);
    List<Connection> connections = singletonList(connection1);
    ...
    assertThat(((MsgStreamer) msgStreamer).getBuffer().capacity())
        .isEqualTo(ConnectionImpl.MAX_MSG_SIZE);
  }
  ...
  protected BaseMsgStreamer createMsgStreamer(boolean mixedDestinationVersions) {
    ...
    when(connection1.getRemoteAddress()).thenReturn(member1);
    when(connection1.getRemoteVersion()).thenReturn(KnownVersion.CURRENT);
    when(connection1.getSendBufferSize()).thenReturn(ConnectionImpl.SMALL_BUFFER_SIZE);
    when(connection2.getRemoteAddress()).thenReturn(member2);
    when(connection2.getSendBufferSize()).thenReturn(ConnectionImpl.SMALL_BUFFER_SIZE);
